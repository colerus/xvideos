...
describe('api/videos/search', () => {
  it('should list search video pages', async () => {
    const list = await videos.search({ key: 'novinha', page: 2 });
  ...
  it('should fail when page parameter is beyond limit', async () => {
    let err;
    try {
      await videos.search({ key: 'novinha', page: Number.MAX_SAFE_INTEGER + 1 });
    } catch (error) {
      err = error;
    } finally {
  ...
  it('should fail when page parameter is less than 1', async () => {
    let err;
    try {
      await videos.search({ key: 'novinha', page: 0 });
    } catch (error) {
      err = error;
    } finally {
      err.should.be.an('error');
