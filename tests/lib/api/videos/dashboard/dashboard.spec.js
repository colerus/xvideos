...
import dashboard from '../../../../../src/lib/api/videos/dashboard/dashboard';
...
describe('api/videos/dashboard', () => {
  ...
  it('should list dashboard video pages', async () => {
    const list = await videos.dashboard({ page: 2 });
  ...
  it('should fail when page parameter is beyond limit', async () => {
    let err;
    try {
      await videos.dashboard({ page: Number.MAX_SAFE_INTEGER + 1 });
    } catch (error) {
      err = error;
    } finally {
